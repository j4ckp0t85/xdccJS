...
export type GlobalMessageEvents = {
   ...
   * @example
   * ```js
   * xdccJS.on('downloading', (fileInfo, received, percentage, eta, rate) => {
   *   console.log(`${fileInfo.file} @ ${received} of ${fileInfo.length} [${percentage}%] - eta: ${eta} - rate: ${rate}`)
   * })
   * ```
   */
  downloading: (fileInfo: FileInfo, received: number, percentage: number, eta: string, rate: string) => void
  /**
...
export default class XDCC extends EventEmitter<GlobalMessageEvents> {
  ...
  private listenCustomEvents(): void {
    ...
    this.irc.on('downloading', (fileInfo, received, percentage, eta, rate) => {
      this.emit('downloading', fileInfo, received, percentage, eta, rate)
    })
  }
   ...
   * })
   */
  download(target: string, packets: Packets, opts?: Partial<{ ipv6: boolean; throttle: number }>): Promise<Job> {
