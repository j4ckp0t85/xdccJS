...
export default class Downloader extends CtcpParser {
  ...
  private processDL(
    ...
    if (fileInfo.position && fileInfo.position > 0) {
      ...
      if (bar) {
        const f = fs.readFileSync(fileInfo.filePath);
        if (f && f.length > 0) {
          bar.tick(f.length, undefined, this.verbose);
        }
      }
    }
    if (throttle) {
  ...
  private onError(args: Pass, e: Error): void {
    ...
    this.SetupTimeout({
      ...
      message: e.message === 'cancel' ? 'Cancelled by user' : `Connection error: %yellow%${e.message}`,
      delay: 0,
      disconnectAfter: {
        stream: args.stream,
        server: args.server,
  ...
  private onData(args: Pass, data: Buffer, sendBuffer: Buffer): void {
    ...
    if (args.bar) args.bar.tick(data.length, undefined, this.verbose);
    ...
    args.candidate.emit(
      ...
      args.fileInfo,
      args.received,
      (args.received / args.fileInfo.length) * 100,
      args.bar?.eta || 'infinite',
      args.bar?.rate || 'NaN'
    );
    this.emit(
      'downloading',
