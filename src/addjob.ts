...
export default class AddJob extends TimeOut {
  ...
  private passMessage(job: Job) {
    const listener = (event: MessageEventArgs) => {
      ...
      if (event.nick === job.nick || event.nick === job.cancelNick) {
        ...
        if (!regex && toMe) {
          this.print(`%yellow%@${job.nick}%reset%: %cyan%${event.message}%reset%`, 8);
        }
      }
    };
  ...
  private static async parsePackets(packets: string | string[] | number | number[]): Promise<number[]> {
  ...
  private static sortPackets(range: number[]): number[] {
    return range.sort((a, b) => a - b).filter((item, pos, ary) => !pos || item !== ary[pos - 1]);
  }
      ...
      if (candidate.queue.length) {
        ...
        if (verbose && candidate.failures.length) {
          ...
          if (candidate.failures.length > 1) {
            ...
            message = `%danger% couldn't download packs: %yellow%${`${firsts.join(
              ', '
            )} and ${last}`}%reset% from %yellow%${candidate.nick}%reset%`;
          }
